...
public class GameJFrame extends JFrame implements KeyListener, ActionListener {
    ...
    public void last()  {
        //获取所有存档的文件的信息
        File f = new File("..\\puzzlegame1\\src\\com\\itheima\\save");
        ...
        for (File file : files) {
            ...
            int index = Integer.parseInt(file.getName().charAt(8) + "");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (obj == replayItem) {
            ...
            String info = item.getText();
            char charNum = info.charAt(2);
            int num = charNum - '0';

            try {
                ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("..\\puzzlegame1\\src\\com\\itheima\\save\\saveData"+num+".data"));
                oos.writeObject(new CCOO(data,step,x,y,path));
                oos.close();
                item.setText("存档"+num +"("+step+")");
                JMenuItem item1 = loadJMenu.getItem(num);
                item1.setText("读档" + num + "("+step+")");
            ...
            CCOO o =null;
            try {
                ObjectInputStream ois = new ObjectInputStream(new FileInputStream("..\\puzzlegame1\\src\\com\\itheima\\save\\saveData"+num+".data"));

                o = (CCOO)ois.readObject();
                ois.close();
            } catch (IOException ex) {
                ex.printStackTrace();
